...
```json
{
  ...
  "payload": {
    "checklist_review": [
      ...
      "observation": "<what the source explicitly says>",
      "gap": "<concrete gap, or why none exists>",
      "impact": "<implementation or verification impact>",
...
When every checklist row includes the v1.3 fields above, the plugin assembles
the persisted fragments into review schema `1.3.0` inside the Host. A
checkpointed review that omits any of those fields is rejected at assembly;
the Host never silently downgrades a new Run to the legacy `1.2.0` envelope.
The Agent does not send this assembled envelope.

Every semantic task also includes `reviewContext.documentContext` and a bounded
`reviewContext.sourceFacts` summary. Use them to understand document type,
scope, delegation, stable identifiers, and explicit assumptions. They are
orientation data; source excerpts and immutable evidence references remain the
authority.
...
- In v1.3, `REWORK` must reference one or more confirmed Findings for the same
  CHK. `NOT_APPLICABLE` requires a delegation or exclusion rationale; an
  unresolved applicability question remains `UNVERIFIED`.
- The Host compares direct absence claims with the frozen `sourceFacts` index.
  A claim that contradicts an observed stable ID or explicit source statement
  is rejected as `INVALID_SEMANTIC_CLAIM` and is not committed.
