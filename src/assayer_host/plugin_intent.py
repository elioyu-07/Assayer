    ...
    *,
    ...
    if operation == "info":
        if plugin_id is None:
            raise IntentResolutionError(
                "INTENT_NEEDS_PLUGIN",
                "Specify which plugin (e.g. 'tell me about spec-quality').",
            )
    ...
    if operation == "downgrade":
        ...
        if plugin_id is None or version is None:
            raise IntentResolutionError(
                "INTENT_NEEDS_DETAIL",
                "downgrade needs a plugin and a target version (e.g. 'pin spec-quality to 1.0.0').",
            )
    ...
    if operation == "rollback":
        if plugin_id is None:
            raise IntentResolutionError(
                "INTENT_NEEDS_PLUGIN",
                "rollback needs a plugin (e.g. 'rollback spec-quality').",
            )
    ...
    if operation == "uninstall":
        if plugin_id is None:
            raise IntentResolutionError(
                "INTENT_NEEDS_PLUGIN",
                "remove/uninstall needs a plugin (e.g. 'remove spec-quality').",
            )
    ...
    if plugin_id is None or check_id is None or scope_file is None:
        raise IntentResolutionError(
            ...
            "(e.g. 'run spec-quality SPEC-001 on spec.md').",
        )
