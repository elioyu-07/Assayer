...
def _legacy_atom(run_id: str, work_item_id: str, check_id: str) -> ReviewAtom:
    return ReviewAtom.create(
        run_id=run_id,
        work_item_id=work_item_id,
        kind="legacy-domain-result",
        source_anchor="complete-work-item",
        rule_id=check_id,
        payload={"compatibilityMode": "complete-work-item"},
    )


def _legacy_verdict(
    run_id: str, work_item_id: str, check_id: str,
    domain_result: Mapping[str, Any], atom: ReviewAtom, batch: ReviewBatch,
) -> BatchVerdict:
    submission_id = _stable_id("review-submission", {
        "runId": run_id,
    ...
    return BatchVerdict(
        ...
        ({
            "atomId": atom.atom_id,
            "disposition": "legacy-complete",
            "domainResult": domain_result,
        },),
    )


def plan_legacy_domain_results(
    *, run_id: str, work_item_ids: Sequence[str], check_id: str,
    max_batch_bytes: int = 24 * 1024,
) -> CoverageLedger:
    """Plan one migration atom per WorkItem before any Agent submission."""
    atoms = tuple(
        _legacy_atom(run_id, work_item_id, check_id) for work_item_id in work_item_ids
    )
    return CoverageLedger.plan(
        run_id, atoms, max_batch_items=1, max_batch_bytes=max_batch_bytes,
    )


def ensure_legacy_domain_results(
    ledger: CoverageLedger | None, *, run_id: str,
    work_item_ids: Sequence[str], check_id: str,
    max_batch_bytes: int = 24 * 1024,
) -> CoverageLedger:
    """Ensure every known legacy WorkItem has one planned review atom."""
    ordered_ids = tuple(work_item_ids)
    if len(ordered_ids) != len(set(ordered_ids)):
        raise PlatformContractError(
            "INVALID_COVERAGE_LEDGER", "Legacy WorkItem identities must be unique",
        )
    if ledger is None:
        return plan_legacy_domain_results(
            run_id=run_id,
            work_item_ids=ordered_ids,
            check_id=check_id,
            max_batch_bytes=max_batch_bytes,
        )
    if ledger.run_id != run_id or ledger.terminal_status is not None:
        raise PlatformContractError(
            "INVALID_REVIEW_TRANSITION", "Legacy plan cannot extend this coverage ledger",
        )
    existing_work_items = {atom.work_item_id for atom in ledger.atoms}
    expanded = ledger
    for work_item_id in ordered_ids:
        if work_item_id in existing_work_items:
            continue
        atom = _legacy_atom(run_id, work_item_id, check_id)
        if _batch_payload_size((atom,), expanded.contexts) > expanded.max_batch_bytes:
            raise PlatformContractError(
                "REVIEW_ATOM_TOO_LARGE", "Legacy review atom exceeds the frozen batch limit",
                work_item_id=work_item_id,
            )
        sequence = len(expanded.batches) + 1
        batch = ReviewBatch(
            _stable_id("review-batch", {
                "runId": run_id, "sequence": sequence, "atomIds": [atom.atom_id],
            }),
            sequence,
            (atom.atom_id,),
            _batch_payload_size((atom,), expanded.contexts),
        )
        expanded = replace(
            expanded,
            atoms=(*expanded.atoms, atom),
            batches=(*expanded.batches, batch),
            entries=(*expanded.entries, CoverageEntry(atom.atom_id, batch.batch_id)),
        )
        existing_work_items.add(work_item_id)
    return expanded


def append_legacy_domain_result(
    ledger: CoverageLedger | None, *, run_id: str, work_item_id: str,
    check_id: str, domain_result: Mapping[str, Any],
    max_batch_bytes: int = 24 * 1024,
) -> CoverageLedger:
    """Append one legacy WorkItem result without replacing earlier batches."""
    if not isinstance(domain_result, Mapping):
        raise PlatformContractError(
            "INVALID_DOMAIN_RESULT", "Legacy DomainResult must be an object",
            work_item_id=work_item_id,
        )
    atom = _legacy_atom(run_id, work_item_id, check_id)
    if ledger is None:
        planned = CoverageLedger.plan(
            run_id, (atom,), max_batch_items=1, max_batch_bytes=max_batch_bytes,
        ).offer()
        return planned.accept(_legacy_verdict(
            run_id, work_item_id, check_id, domain_result, atom, planned.batches[0],
        ))
    if ledger.run_id != run_id or ledger.terminal_status is not None:
        raise PlatformContractError(
            "INVALID_REVIEW_TRANSITION", "Legacy result cannot append to this coverage ledger",
        )
    existing_atom = next((item for item in ledger.atoms if item.atom_id == atom.atom_id), None)
    if existing_atom is not None:
        batch = next(item for item in ledger.batches if atom.atom_id in item.atom_ids)
        verdict = _legacy_verdict(
            run_id, work_item_id, check_id, domain_result, atom, batch,
        )
        existing_submission = next(
            (item for item in ledger.verdicts if item.submission_id == verdict.submission_id), None,
        )
        if existing_submission is not None:
            return ledger.accept(verdict)
        if batch.status in {"planned", "offered"}:
            offered = ledger.offer(batch.batch_id) if batch.status == "planned" else ledger
            return offered.accept(verdict)
        raise PlatformContractError(
            "REVIEW_SUBMISSION_CONFLICT",
            "A different legacy DomainResult already covers this WorkItem",
            work_item_id=work_item_id,
        )
    if _batch_payload_size((atom,), ledger.contexts) > ledger.max_batch_bytes:
        raise PlatformContractError(
            "REVIEW_ATOM_TOO_LARGE", "Legacy review atom exceeds the frozen batch limit",
            work_item_id=work_item_id,
        )
    sequence = len(ledger.batches) + 1
    batch = ReviewBatch(
        _stable_id("review-batch", {
            "runId": run_id, "sequence": sequence, "atomIds": [atom.atom_id],
        }),
        sequence,
        (atom.atom_id,),
        _batch_payload_size((atom,), ledger.contexts),
    )
    expanded = replace(
        ledger,
        atoms=(*ledger.atoms, atom),
        batches=(*ledger.batches, batch),
        entries=(*ledger.entries, CoverageEntry(atom.atom_id, batch.batch_id)),
    ).offer(batch.batch_id)
    return expanded.accept(_legacy_verdict(
        run_id, work_item_id, check_id, domain_result, atom, batch,
    ))


def adapt_legacy_domain_result(
    *, run_id: str, work_item_id: str, check_id: str,
    domain_result: Mapping[str, Any], max_batch_bytes: int = 24 * 1024,
) -> CoverageLedger:
    """Represent one complete legacy DomainResult as an accepted single batch."""
    return append_legacy_domain_result(
        None,
        run_id=run_id,
        work_item_id=work_item_id,
        check_id=check_id,
...
__all__ = [
    ...
    "ReviewAtom", "ReviewBatch", "adapt_legacy_domain_result",
    "append_legacy_domain_result", "ensure_legacy_domain_results",
