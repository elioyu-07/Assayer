...
KNOWN = ("assayer.spec-quality", "assayer.config-quality", "assayer.frontend-audit")
...
class PluginIntentResolutionTest(unittest.TestCase):
    ...
    def test_resolves_info_intent_by_short_name(self):
        plan = resolve_intent("tell me about spec-quality", known_plugin_ids=KNOWN)
        self.assertEqual(len(plan), 1)
        self.assertEqual(plan[0].operation, "info")
        self.assertEqual(plan[0].plugin_id, "assayer.spec-quality")
    ...
    def test_resolves_downgrade_with_version(self):
        plan = resolve_intent("pin spec-quality to 0.9.0", known_plugin_ids=KNOWN)
    ...
    def test_resolves_rollback_and_uninstall(self):
        self.assertEqual(
            resolve_intent("undo the last update for spec-quality", known_plugin_ids=KNOWN)[0].operation,
            "rollback",
        )
        self.assertEqual(
            resolve_intent("remove spec-quality", known_plugin_ids=KNOWN)[0].operation,
            "uninstall",
        )
    ...
    def test_run_requires_check_and_scope_file(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            plan = resolve_intent(
                f"run spec-quality SPEC-001 on {scope}", known_plugin_ids=KNOWN,
            )
    ...
    def test_downgrade_without_version_asks_for_detail(self):
        with self.assertRaises(IntentResolutionError) as ctx:
            resolve_intent("downgrade spec-quality", known_plugin_ids=KNOWN)
        self.assertEqual(ctx.exception.code, "INTENT_NEEDS_DETAIL")

    def test_install_bare_name_is_unresolved(self):
        with self.assertRaises(IntentResolutionError) as ctx:
            resolve_intent("install spec-quality", known_plugin_ids=KNOWN)
