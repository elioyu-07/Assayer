"""The external Spec-quality distribution completes the full M4 exit gate.

The journey is recorded by ``scripts/spec_plugin_external_acceptance.py``:
...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "spec_plugin_external_acceptance.py"
SPEC = importlib.util.spec_from_file_location("spec_plugin_external_acceptance", SCRIPT)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("spec external journey script cannot be loaded")
spec_plugin_external_acceptance = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(spec_plugin_external_acceptance)


class SpecQualityExternalJourneyTest(unittest.TestCase):
    def test_full_external_journey_completes_the_m4_exit_gate(self):
        result = spec_plugin_external_acceptance.run()
        self.assertEqual(result["status"], "passed")
        self.assertFalse(result["publishable"])
        self.assertEqual(result["pluginId"], "assayer.spec-quality")
        self.assertEqual(
