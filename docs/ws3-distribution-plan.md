...
| Phase | 1 — freeze the distributions and the dependency DAG |
...
- **`assayer_agent` is folded into `assayer-platform` for now.** It is
  ...
  platform-only.
- **The root `assayer` meta package is untouched in phase 1.** It still
  bundles every module and declares every entry point; the flip is phase 3,
  gated on a green install matrix.
