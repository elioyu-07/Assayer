...
| 文档版本 | 1.0.0-draft |
| 日期 | 2026-08-30 |
| 状态 | implementation-ready；当前完成 B07a/B07b、B08、B09，B07c 脱敏增强暂缓 |
...
B07 拆分为 B07a、B07b 与 B07c：B07a/B07b 已完成，B07c 暂缓。B07b 只证明“能安全捕获并绑定原始对象图片”，不证明图片已脱敏；在 B07c 完成前，真实路径仍不能生成正式 `issue_found`。B08 已完成 JSON/MCP 传输，B09 已完成发布级故障注入；传输层和故障处理均不能绕过截图或脱敏门禁。
