...
| 文档版本 | 1.0.0-draft |
| 日期 | 2026-08-30 |
| 状态 | implementation-ready；当前完成 B07a，B07b 暂缓，下一项 B08 |
...
| B07a | 真实结构化 Evidence | DOM/ARIA 最小事实、对象/Case/页面绑定、字段值分类与结构化脱敏 | 已完成（垂直切片 017）；原始 DOM、字段原值和秘密不落账，不产生图片文件 |
| B07b | 真实截图与像素脱敏（暂缓） | 对象截图、敏感区域识别、不可逆像素遮挡 | 2026-08-30 按产品决策暂不实现；真实浏览器路径继续禁止正式 `issue_found` |
...
B07 拆分为 B07a 与 B07b：B07a 已完成，下一项推进 B08；B07b 暂缓。B04 之后才允许声称“可读取真实页面”；B08 只能传输结构化事实和非正式结论，不能绕过截图门禁；只有 B07b 完成后才允许生成真实正式 `issue_found`。B09 可以开展不依赖截图的故障注入，但 B07b 未完成时不能进入发布候选。

## 5. B07 拆分与 B07b 暂缓决策

- 决策日期：2026-08-30。
- 继续范围（B07a）：真实 Chromium DOM/ARIA 最小事实采集、对象/Case/PageState 强绑定、字段状态分类、结构化脱敏和完整性摘要。
- 暂缓范围（B07b）：真实 Chromium 截图采集、对象裁剪、敏感区域识别、不可逆像素遮挡及脱敏确认。
- 不变安全门禁：不删除 Screenshot Schema，不降低 `issue_found` 的独立截图要求，不用未脱敏图片或 deterministic fixture 冒充真实证据。
- 暂缓期间允许：B07a 已完成；继续实现 B08 的通用 JSON CLI/MCP 传输，以及 B09 中不依赖真实截图的安装、超时、崩溃、冲突和传输故障测试。
- 暂缓期间禁止：真实站点生成、提交或发布 `issue_found`；对外宣称已具备真实视觉取证能力；把整页截图、浏览器录屏或人工截图作为自动脱敏的替代品。
- 恢复 B07b 时仍按原验收门槛执行，不因暂缓降低像素脱敏、对象对位和失败关闭要求。
