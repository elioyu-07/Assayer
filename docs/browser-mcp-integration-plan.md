...
| 文档版本 | 1.0.0-draft |
| 日期 | 2026-08-30 |
| 状态 | implementation-ready；当前完成 B06，B07 暂缓，下一项 B08 |
...
| B07 | 真实 Evidence 与截图脱敏（暂缓） | DOM 最小证据、对象截图、敏感区域像素遮挡 | 2026-08-30 按产品决策暂不实现；真实浏览器路径继续禁止正式 `issue_found` |
...
除已明确暂缓的 B07 外，任务仍按顺序推进。B04 之后才允许声称“可读取真实页面”；B08 可以先完成传输接入，但只能传输结构化事实和非正式结论，不能绕过截图门禁生成真实正式问题；只有 B07 完成后才允许生成真实正式 `issue_found`。B09 可以开展不依赖截图的故障注入，但 B07 未完成时不能进入发布候选。

## 5. B07 暂缓决策

- 决策日期：2026-08-30。
- 暂缓范围：真实 Chromium 截图采集、对象裁剪、敏感区域识别、不可逆像素遮挡及脱敏确认。
- 不变安全门禁：不删除 Screenshot Schema，不降低 `issue_found` 的独立截图要求，不用未脱敏图片或 deterministic fixture 冒充真实证据。
- 暂缓期间允许：继续实现 B08 的通用 JSON CLI/MCP 传输，以及 B09 中不依赖真实截图的安装、超时、崩溃、冲突和传输故障测试。
- 暂缓期间禁止：真实站点生成、提交或发布 `issue_found`；对外宣称已具备真实视觉取证能力；把整页截图、浏览器录屏或人工截图作为自动脱敏的替代品。
- 恢复 B07 时仍按原验收门槛执行，不因暂缓降低像素脱敏、对象对位和失败关闭要求。
