...
| B12 | 通用真实 URL 全链路验收 | 规则驱动的 Case/动作/Evidence/恢复/判定/账本运行器；结构化 Evidence 事实通道 | 已完成（垂直切片 023）；不依赖站点文案或专属选择器，真实 URL 可完成 Host 生命周期；证据不足保守为 `needs_review`，账本与派生报告闭合 |
