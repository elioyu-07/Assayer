...
- B07a/B07b 已完成，真实浏览器可以生成结构化 Evidence 和对象级 Raw Visual；B07b 图片会明确标记 `sanitizationStatus=not_performed`，因此所有真实 `issue_found` 仍被脱敏门禁拒绝，B08 传输层不得改变该结果；B08 的 JSON CLI 与 MCP-shaped 适配都只委托同一个 `HostCore.handle`；
