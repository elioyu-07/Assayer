...
3. 识别常见业务待检查对象；
4. 按已确认的 14 条规范生成反向 Case，并优先检查边界、非法和异常场景；
5. 结合页面运行态、可用源码、请求和视觉证据进行判断；
6. 对证据充分的问题自动生成正式问题项；
7. 为每个正式问题提供与页面病灶对应的独立截图；
8. 输出问题报告、完整审计账本、覆盖证明和失败/部分完成诊断。

第一版不承诺发现 14 条规范之外的全部前端问题。规范之外的明显问题只能作为“额外观察/新规则建议”展示，不计入正式问题结论。

14 条规范只是第一版基线，不是产品能力上限。agent-f 必须支持后续持续增加、修改、停用和版本化审计规范，而不要求重写 Agent 调查流程、Host 核心、主账本或报告模型。
...
- 审计流程和 Agent 行为规范；
- 14 条有效规范及其反向 Case 原则；
- 对象类型和规范适用性知识；
- 通过、问题、噪声和待复核的判断标准；
- 证据要求、反例和覆盖要求；
- 输出术语和报告格式。

你是 14 条规范的最终 Owner。Agent 可以访谈和整理规则草案，但未获你确认的规则不能用于自动确认问题。
...
- `record_decision`：记录对象/规范结果和证据引用；
