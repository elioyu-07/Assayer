...
| Date | 2026-09-13 |
| Status | Current implementation baseline plus Simple authoring migration gates |
...
| Frozen typed source boundary | Constitution §3.16; Plugin Contract §5 | HostDocumentSource freezes document snapshots; the browser provider freezes BrowserSnapshot input and owns provider-bound WorkItems; the former Frontend runtime adapter has been removed | Implemented for ordinary plugins | Add resume-time invalidation cases for each new source provider |
...
| Canonical result and derived reports | Canonical Result Contract §1 | Generic batch and interactive Runs derive and schema-validate `canonical-result.json`; compiler-generated Frontend uses common review and the platform report renderer; exact source-ledger digests are checked | Implemented | Prove historical migration fixtures when a source-ledger schema version changes |
...
documentation. Frontend has completed the Policy Pack hard cut and no longer
has an Advanced SPI compatibility implementation. Remaining Partial rows are
platform work and do not authorize restoring an ordinary-plugin fallback.
