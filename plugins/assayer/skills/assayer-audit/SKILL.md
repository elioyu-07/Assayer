...
4. Inspect the page and verified objects before choosing rules. For frontend presentation rules, do not demand backend or source proof unless the frozen rule explicitly requires it.
...
7. Prefer the atomic `prepare_decision` form: include the complete `findings` array with the object/rule, final result, evidence/case refs, and reason. The Facade performs Finding recording, preparation, and commit on the owning Host thread. If using legacy calls, finish `record_findings → prepare_decision → commit_decision` before changing pages; Host blocks navigation while an object decision is open.
...
10. During a long audit, send a short user-facing progress update after startup and at meaningful phase changes (page discovery, object checks, recovery, and finalization). Do not claim progress that is not supported by the latest Host response.
